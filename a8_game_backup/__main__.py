    ...
    if choice == "1":
            ...
            for readlist in select.select(poll_list,[],[], 0.1):
                for read in readlist:
                    ...
                    if read == server_socket.fileno():
                        data, addr = server_socket.recvfrom(1024)
                        if data == b"ping":
                            server_socket.sendto(f"pong f{hostname}") # .encode()
                            continue
                        print(data)
                        if addr in hosts:
                            continue
                        ...
                        for client in hosts:
                            if client != addr:
                                server_socket.sendto(b"\x89" + json.dumps(addr).encode(), client)
                        #server_socket.sendto(json.dumps(hosts).encode(), addr)
                        #hosts.append(addr)
        ...
        while True:
            ...
            if data.startswith(b"\x89"):
                newcomer = hosts.append(tuple(json.loads(data[1:].decode())))
                newcomer = tuple(json.loads(data[1:].decode()))
                if newcomer not in hosts:  # don’t duplicate
                    hosts.append(newcomer)
                print("New peer joined:", newcomer)
